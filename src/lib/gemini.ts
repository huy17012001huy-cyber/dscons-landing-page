 ...
 */
export async function analyzeCompetitor(studentData: any, studentNeed: string = "", studentHesitation: string = "", retryCount: number = 0): Promise<{ aiData: any }> {
  try {
...
${studentHesitation ? `\nĐIỂM PHÂN VÂN LỚN NHẤT CỦA HỌC VIÊN:\n"${studentHesitation}"\n-> TRƯỚC KHI kết luận, hãy dành một phần để phân tích điểm phân vân này. Đưa ra lập luận khách quan để giải quyết điểm phân vân này.\n` : ""}
...
TRẢ VỀ KẾT QUẢ DƯỚI DẠNG JSON THEO ĐÚNG CẤU TRÚC SAU (Tuyệt đối không Markdown, không bọc HTML bên ngoài, chỉ trả về JSON hợp lệ):
{
  ...
  "conclusionPoints": [
    "Điểm kết luận 1",
    ${studentHesitation ? `"Điểm phân tích thứ 2: Phân tích trực diện ĐIỂM PHÂN VÂN của học viên (Nêu rõ lợi/hại dựa trên dữ liệu so sánh)",` : ""}
...
`;
    ...
    try {
      ...
      if (retryCount < 2) {
        ...
        return await analyzeCompetitor(studentData, studentNeed, studentHesitation, retryCount + 1);
      }
